package com.example.healthconnect.utilty.impl.ui.editor.record
...
class PowerEditor() :Editor<PowerRecord, Power>() {
    ...
    override fun toRecord(
        ...
        samples = validModel.samples.items.map {
            PowerRecord.Sample(
                time = it.time,
                power = androidx.health.connect.client.units.Power.watts((it.power as ValueField.Dbl).parsedValue!!)
            )
        }
    )
