...
internal class TimeFieldEditorViewModel(
    model: TimeField,
) : ViewModel() {
    ...
    fun onEvent(event: Event) {
        ...
        onTimeChangedJob = viewModelScope.launch {
            val model = when (event) {
                is Event.OnInstantaneousTimeChanged -> TimeField.Instantaneous(
                    time = timeModel(
                ...
                is Event.OnEndTimeChanged -> (_state as TimeField.Interval).copy(
                    end = timeModel(
                        instantValue = event.value,
                        zoneValue = event.zoneId,
                    )
                ...
                is Event.OnStartTimeChanged -> (_state as TimeField.Interval).copy(
                    start = timeModel(
                        instantValue = event.value,
                        zoneValue = event.zoneId,
                    )
    ...
    companion object {
        ...
        val TIME_MODEL_KEY: CreationExtras.Key<TimeField> = CreationExtras.Companion.Key()
    }
}